    ...
    <header className="fixed top-0 text-center w-full z-50 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-200">
      <div className="mx-auto max-w-7xl px-2 align-middle ">
        <span className="footer-text text-xs">
          ...
          {" | "}
          <Link to="https://github.com/ericvolp12/atproto.tools" target="_blank" className="inline-block h-3 w-3 my-auto fill-slate-600 dark:fill-slate-200" >
...
const ThemeWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return (
    <div className="bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-200 min-h-dvh">
      {children}
    </div>
  );
}
